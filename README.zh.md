...
- **评估指标：** WIS、AE median、95% 区间覆盖率（绝对分数）
...
| Hub 目标数据同步 | 周五 02:00 UTC / 周日 06:00 UTC（Hub `sync-target-data.yaml`） |
| Dashboard 数据更新 | 周五 04:00 UTC / 周日 08:00 UTC（自动，`regenerate=true`；在 Hub 同步之后） |
